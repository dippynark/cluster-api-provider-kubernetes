...
func (input *ControlplaneClusterInput) ControlPlaneCluster() {
	...
	matchClusterListOption := client.MatchingLabels{clusterv1.MachineClusterLabelName: input.Cluster.Name}
...
func (input *ControlplaneClusterInput) CleanUpCoreArtifacts() {
	...
	lbl, err := labels.Parse(fmt.Sprintf("%s=%s", clusterv1.MachineClusterLabelName, input.Cluster.GetClusterName()))
